# Probably best to make functions for all this, QA script style
function SelectDisk {
    Clear-Host
...
function InstallNixOS {
    ...
    # Copy-Item -Path "/mnt/etc/nixos/hardware-configuration.nix" -Destination "/etc/nixos/hardware-configuration.nix" -Recurse -Force # the currently booted environment needs a hardware-configuration.nix to run nixos-install
